...
object SramExtraPortDirection extends Enumeration {
  type SramExtraPortDirection = Value
  val In, Out = Value
}

case class SramPort(name: String, priority: SramPortPriority.Value = SramPortPriority.High)
...
object SramPort {
  implicit val rw: Reader[SramPort] = reader[ujson.Value].map[SramPort](json => {
    val portPriority = json.obj.getOrElse("priority", default = ujson.Str("HIGH")).str match {
      case "LOW"  => SramPortPriority.Low
      case "HIGH" => SramPortPriority.High
...
case class SramExtraPort(name: String, direction: SramExtraPortDirection.Value, constant: String = null) {
  direction match {
    case SramExtraPortDirection.In  => require(constant != null, "Constant value should be nonnull for input port.")
    case SramExtraPortDirection.Out => require(constant == null, "Constant value should be null for output port.")
  }
}
object SramExtraPort {
  implicit val rw: Reader[SramExtraPort] = reader[ujson.Value].map[SramExtraPort](json => {
    val direction: SramExtraPortDirection.Value = json("direction").str.toLowerCase() match {
      case "in"  => SramExtraPortDirection.In
      case "out" => SramExtraPortDirection.Out
...
/** Configuration for SRAM compilation.
  ...
  * @param nameRule
  *   String to specify the name rule of the SRAM. The following keywords: $depth$, $width$, $mux$, will be replaced
  *   with actual parameters when elaboration.
  * @param maskUnit
  ...
  * @param extraPorts
  *   A List of [[SramExtraPort]] to describe the extra ports. Extra ports can only be tied to constant values.
  */
